...
func _draw():
	var radius := blast_radius()
	if state != "rolling":
		draw_arc(Vector2.ZERO,radius,0,TAU,48,Color(1,0.9,0.65,0.35),1.5)
		for i in range(pips):
	...
	var lift = -absf(sin(age*12))*12 if state == "rolling" else -2.0
	...
	if state != "rolling":
		var label = "ROYAL SIX!" if royal else "%d  •  RETURN CROWN" % pips
